...
impl TryFrom<AST> for ArgPattern {
    ...
    fn try_from(ast: AST) -> Result<Self, Self::Error> {
        Ok(match ast {
            ...
            AST::Group(x) => ArgPattern::try_from(x.item)?,
            AST::Repeat(r) => ArgPattern::Repeat(Box::new(r.map(ArgPattern::try_from)?)),
