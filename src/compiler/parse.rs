...
impl Parser {
    ...
    pub fn repeat(&mut self) -> Result<Spanned<AST>, Syntax> {
        let token = self.consume(Token::Repeat)?.to_owned();
        let expression = self.expression(Prec::Call, false)?;
        let span = Span::combine(&token.span, &expression.span);
        let tree = AST::Repeat(Box::new(expression));
        Ok(Spanned::new(tree, span))
    }
