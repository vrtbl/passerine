...
/// to an `AST` slice.
type Bindings = HashMap<String, Vec<Spanned<AST>>>;
...
impl Rule {
    ...
    pub fn bind(
        ...
        match &arg_pat.item {
            ...
            ArgPattern::Symbol(symbol) => {
                Some(Ok(vec![(symbol.clone(), vec![reversed_form.pop()?])]
                    .into_iter()
                    .collect()))
            }
            ArgPattern::Group(pats) => {
            ...
            ArgPattern::Repeat(pattern) => {
                let so_far: Bindings = HashMap::new();
                while let Some(x) = Self::bind(arg_pat, reversed_form) {
                    if x.is_err() {
                        break;
                    }
                    for (key, value) in x.unwrap().into_iter() {
                        if let Some(vec) = so_far.get_mut(&key) {
                            // TODO: how to handle this
                            assert_eq!(vec.len(), 1);
                            vec.append(&mut value)
                        } else {
                            so_far.insert(key, value);
                        }
                    }
                }
    ...
    pub fn resolve_symbol(name: String, span: Span, bindings: &mut Bindings) -> Spanned<AST> {
        if let Some(bound_tree) = bindings.get(&name) {
            ...
            bindings.insert(name, vec![spanned]);
    ...
    pub fn expand_arg_pat(
        ...
        Ok(match arg_pat.item {
            ...
            ArgPattern::Repeat(inner) => {
                let span = arg_pat.span.clone();
                Spanned::new(
                    ArgPattern::Repeat(Box::new(Rule::expand_arg_pat(*inner, bindings)?)),
                    span,
                )
            }
        })
    ...
    pub fn expand(tree: Spanned<AST>, mut bindings: &mut Bindings) -> Result<Spanned<AST>, Syntax> {
        ...
        let item: AST = match tree.item {
            ...
            AST::Repeat(expression) => {
                let x = ();
                return Rule::expand(*expression, bindings);
            }
