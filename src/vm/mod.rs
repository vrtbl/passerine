...
use crate::vm::{slot::Suspend, stack::Stack, trace::Trace};
...
impl VM {
    ...
    pub fn ffi_call(&mut self) -> Result<(), Trace> {
        ...
        let returned = match ffi_function.call(argument) {
            Ok(d) => d,
            Err(e) => return Err(Trace::error("FFI Call", &e, vec![self.current_span()])),
        };
...
mod test {
    ...
    use crate::compiler::{desugar::desugar, gen::gen, hoist::hoist, lex::lex, parse::parse};
